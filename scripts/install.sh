...
if [ $ERR -ne 0 ]; then
    echo "Error while checking preliminary Globaleaks requirements: $ERR missing"
    exit
else
    echo "Preliminary requrements completely meet"
fi
...
VIRTUALENV_SIG_URL="https://pypi.python.org/packages/source/v/virtualenv/$VIRTUALENV_PKG.asc"
VIRTUALENV_MD5="07e09df0adfca0b2d487e39a4bf2270a"
VIRTUALENV_PUB_KEY="-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: antani 2.0
...
echo "$VIRTUALENV_MD5  $VIRTUALENV_PKG" > $TARGET_PATH/building/MD5SUMS
echo "Checking if the md5 sum is what we expect"
md5sum --check $TARGET_PATH/building/MD5SUMS
if [ $? -ne 0 ]; then
  echo "Error in verifying MD5 sum!"
  exit
fi
rm $TARGET_PATH/building/MD5SUMS

echo "Verifying PGP signature"
TMP_KEYRING=$TARGET_PATH/building/tmpkeyring.gp
PKG_VERIFY=$TARGET_PATH/building/$VIRTUALENV_PKG.asc
curl -O $VIRTUALENV_SIG_URL
echo "$VIRTUALENV_PUB_KEY" | gpg --no-default-keyring --keyring $TMP_KEYRING --import
gpg --no-default-keyring --keyring $TMP_KEYRING --verify $PKG_VERIFY
if [ $? -ne 0 ]; then
  echo "Error in verifying signature!"
  exit
fi
tar xvf $TARGET_PATH/building/$VIRTUALENV_PKG
python virtualenv*/virtualenv.py --no-site-packages $DEPLOY_PATH/glenv
if [ $? -ne 0 ]; then
...
source $DEPLOY_PATH/glenv/bin/activate

echo "Installing dependencies"
pip install -r $DEPLOY_PATH/$TARGET_GIT_REPO/GLBackend/requirements.txt
if [ $? -ne 0 ]; then
